...
class GPTConfig:
    ...
    n_embd: int = 768
    ffn_mult: int = 4
    # Fraction of layers, starting from layer 0, that use PostLN normalization.
    # 0.0 keeps all blocks in the standard PreLN-style GPT form; 1.0 uses PostLN
    # for all blocks. Intermediate values are MixLN ablations.
...
class Block(nn.Module):
    def __init__(self, config, use_post_ln: bool = False):
        super().__init__()
    ...
    def extra_repr(self) -> str:
        return f"norm_type={'PostLN' if self.use_post_ln else 'PreLN'}"

    def forward(self, x):
        if self.use_post_ln:
            # PostLN: x = norm(x + sublayer(x))
            x = F.rms_norm(x + self.attn(x), (x.size(-1),))
            x = F.rms_norm(x + self.mlp(x), (x.size(-1),))
        else:
            # PreLN: x = x + sublayer(norm(x))
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        num_postln_layers = int(config.n_layer * config.postln_frac)

        self.transformer = nn.ModuleDict(
            dict(
                wte=nn.Embedding(config.vocab_size, config.n_embd),
                h=nn.ModuleList(
                    [Block(config, use_post_ln=(i < num_postln_layers)) for i in range(config.n_layer)]
                ),
            )
        )
