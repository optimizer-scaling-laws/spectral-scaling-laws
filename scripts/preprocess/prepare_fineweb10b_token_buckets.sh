...
FREQ_FILE="${FREQ_FILE:-results/processed/token_frequencies.pt}"
FREQ_NPY_FILE="${FREQ_NPY_FILE:-results/processed/token_frequencies.npy}"
...
python scripts/preprocess/compute_token_frequencies.py   --data_dir "${DATA_DIR}"   --output "${FREQ_FILE}"   --npy_out "${FREQ_NPY_FILE}"   --json_out "${STATS_FILE}"
