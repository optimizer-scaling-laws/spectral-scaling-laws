...
The training entrypoint calls `seed_everything(seed)` before model initialization, dataloader construction, and optimizer construction. The paper used one training run per configuration rather than a multi-seed sweep. Therefore, analysis outputs should not describe confidence intervals as seed confidence intervals unless future multi-seed runs are added.

For the paper's original logs where the seed was not recorded explicitly, the analysis metadata should use `seed=not_recorded` rather than fabricating a value.
...
- `rank0_local`: rank-0 local bucket metrics, matching the paper's experiments.
- `distributed_covariance`: globally reduced bucket covariance statistics for new runs.

Paper configs use `rank0_local` to preserve compatibility with the paper's runs.
...
The paper's full raw logs, checkpoints, and large intermediate per-layer/per-step CSVs are external artifacts. The committed repository is designed to reproduce figures from compact processed CSVs, and to parse raw logs when the external raw-log bundles are supplied.
