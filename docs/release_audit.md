...
| Special figure families | Processed CSVs plus full launch-config grids for Dion rank sweep, matched-loss, 350M TAIL, and architecture-vs-optimizer | Their committed PDFs; the paper's raw logs remain external |
| Training launch examples | Configs and shell wrappers | New runs with the released code; the paper's original seeds/logs are not fully bundled |
...
- The architecture-vs-optimizer figure is regenerated from processed beta values, and the full 80-run 12-head/6-head launch-config grid is committed. The paper's raw logs remain external.
- Dion rank-sweep, matched-loss, GPT2-350M TAIL, and architecture-vs-optimizer figure families are reproducible from processed CSVs in this repo and now have full launch-config grid coverage. Rebuilding their processed CSVs from the paper's raw logs still requires the external raw-log bundles.
- The paper's original runs should use `seed=not_recorded` in metadata. Released launch configs include `seed: 1337` for future reproducibility.
